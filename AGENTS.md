...
聊天页录音按钮用于开启/关闭连续语音输入。连续语音输入由前端浏览器常驻麦克风监听和 VAD 音量检测实现：检测到用户开口时立刻打断当前 OpenAI 流、TTS 合成和 Web Audio 播放；检测到句尾静音后，将该段录音发送到现有 `/api/asr`，再自动进入 `/api/chat/stream`。不要把 ASR 改成后端常驻阻塞进程，除非用户明确要求。
