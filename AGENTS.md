...
`text_display_mode=speech_sync` 且启用 GSV 语音时，后端 OpenAI producer 必须对 TTS worker 做背压，避免 OpenAI 快速读完整段回复后才慢慢排队合成。切出一个可合成 TTS 段后，应限制最多一个段处于合成/等待合成状态；该段处理完成后再继续读取后续 OpenAI 文本。`text_first` 模式不需要该背压。
