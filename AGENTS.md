...
OpenAI 聊天采样参数使用 `openai_` 前缀保存，避免和 GSV TTS 的 `top_k/top_p/tts_temperature` 混淆。`openai_top_k`、`openai_repetition_penalty` 等兼容服务字段必须允许留空；留空时不要发送给上游 chat/completions。
