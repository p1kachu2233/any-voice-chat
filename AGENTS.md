...
聊天页录音按钮用于开启/关闭连续语音输入。连续语音输入由前端浏览器常驻麦克风监听和 VAD 音量检测实现，VAD 要使用动态底噪和连续命中帧，不要只靠过低的单帧 RMS 阈值；VAD 参数必须通过后台应用设置页可配置。确认用户开始录音时同步打断当前 OpenAI 流、TTS 合成、ASR 请求和 Web Audio 播放。监听开启后 MediaRecorder 应常驻分片并维护预录缓冲，确认开口时把前置音频一起送 ASR，避免句首被吞。不要启用浏览器 Web Speech API 做实时临时字幕，它容易累积错误文本并拾取助手外放声音；检测到句尾静音后，将该段录音发送到现有 `/api/asr`，用后端 ASR 最终文本覆盖草稿气泡，再自动进入 `/api/chat/stream`。不要把 ASR 改成后端常驻阻塞进程，除非用户明确要求。
