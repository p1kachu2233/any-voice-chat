...
聊天页录音按钮用于开启/关闭连续语音输入。连续语音输入默认使用前端本地 `vad-web` + Silero ONNX 模型做人声检测，相关 JS、ONNX、WASM 资源放在 `app/static/vendor/`，页面运行时必须可离线加载，不依赖 CDN；`rms` 音量检测只作为兼容备用。`vad-web` 参数使用模型概率和帧处理语义，配置键统一使用 `vad_web_*`，如 `vad_web_positive_threshold`、`vad_web_negative_threshold`、`vad_web_redemption_ms`、`vad_web_pre_speech_pad_ms`、`vad_web_min_speech_ms`；RMS 参数使用音量阈值、动态底噪、连续命中帧、静音时长等，配置键统一使用 `rms_vad_*`。两套配置不要混用，也不要在网页里展示已经不生效的旧泛名 `vad_*` 字段。确认用户开始录音时同步打断当前 OpenAI 流、TTS 合成、ASR 请求和 Web Audio 播放。不要启用浏览器 Web Speech API 做实时临时字幕，它容易累积错误文本并拾取助手外放声音；检测到句尾静音后，将该段录音发送到现有 `/api/asr`，用后端 ASR 最终文本覆盖草稿气泡，再自动进入 `/api/chat/stream`。不要把 ASR 改成后端常驻阻塞进程，除非用户明确要求。连续录音不要拼接长期 `MediaRecorder` 的中间 WebM 分片作为 ASR 输入；中间分片可能缺少容器头，导致 ffmpeg `pipe:0 Invalid data`。`vad-web` 返回的 16k PCM 应转成本地 WAV Blob 送 ASR；`rms` 备用模式如需使用 MediaRecorder，应按单句话新建 recorder。
...
Qwen 等 OpenAI 兼容思考模型使用 `openai_thinking_mode` 控制，取值为 `auto/on/off`。`auto` 表示不发送扩展字段；`on/off` 才分别向上游请求体发送 true/false。思考参数格式由 `openai_thinking_format` 控制：`chat_template_kwargs` 发送 `{"chat_template_kwargs":{"enable_thinking":...}}`，`root` 发送顶层 `enable_thinking`。默认使用 `chat_template_kwargs`，避免把顶层未知字段强行发给普通 OpenAI 服务。
...
`text_display_mode=speech_sync` 且启用 GSV 语音时，后端 OpenAI producer 必须对 TTS worker 做小缓冲背压，避免 OpenAI 快速读完整段回复后把所有句子堆进 TTS 队列。切出一个可合成 TTS 段后应立即送入 TTS 队列；队列只允许少量预缓冲（当前为 1 段），不要等待当前段完整合成结束才继续读取下一段，也不要让整段回复无限入队。`text_first` 模式不需要该背压。
