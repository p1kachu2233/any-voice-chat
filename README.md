...
聊天页顶部的 `语音` 开关用于启用或关闭 GSV 语音合成。开启时会先检查 GSV 是否已连接；关闭后只进行文字聊天。聊天时文本会逐字显示，语音会通过 GPT-SoVITS TTS 代理边接收边播放。Streaming Mode 默认使用 `1`；`1/2/3` 是 GSV 的生成流式模式，`0` 会完整生成后再通过同一个流式代理传输。

GSV 第一次合成通常会较慢，可以在后台设置页点击 `预热 GSV`，提前跑一次短文本合成。
